...
class UNetGenerator(nn.Module):
    def __init__(self, in_ch=1, out_ch=3):
        super().__init__()
        # Encoder
        self.down1 = self.contract(in_ch, 64, norm=False)   # 1 → 64
        self.down2 = self.contract(64, 128)                 # 64 → 128
        self.down3 = self.contract(128, 256)                # 128 → 256
        self.down4 = self.contract(256, 512)                # 256 → 512
        self.down5 = self.contract(512, 512)                # 512 → 512
        self.down6 = self.contract(512, 512)                # 512 → 512

        # Decoder (reverse order)
        self.up1 = self.expand(512, 512, dropout=True)      # 512 → 512
        self.up2 = self.expand(1024, 512, dropout=True)     # (512+512) → 512
        self.up3 = self.expand(1024, 256)                   # (512+512) → 256
        self.up4 = self.expand(512, 128)                    # (256+256) → 128
        self.final_up = self.expand(256, 64)                # (128+128) → 64
...
for epoch in range(epochs):
    ...
    for sketch, photo in tqdm(train_loader):
        ...
        # FIXED — label size matches discriminator output
    ...
    print(f"💾 Saved models → epoch {epoch+1}")
