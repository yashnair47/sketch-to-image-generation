...
# ============================================================
# 🎨 Improved Realistic AI Output Simulation
# ============================================================
def generate_images(sketch, sketch_file):
    if sketch is None or sketch_file is None:
        return None, None, None, None, None, None, "⚠️ Please upload a valid sketch image.", gr.update(visible=False)
    ...
    if not os.path.exists(photo_path):
        return sketch_resized, None, None, None, None, None, f"❌ No matching photo found for `{fname}` in Photos folder.", gr.update(visible=False)
    ...
    # 1️⃣ Pix2Pix — best quality
# ============================================================
# Simulated Model Outputs (with light blur adjustment)
# ============================================================

# 1️⃣ Pix2Pix — best quality, but slightly softened
    pix2pix = real.copy()
    ...
    pix2pix = pix2pix.filter(ImageFilter.GaussianBlur(0.5))  # ✅ slight soft blur

# 2️⃣ Diffusion — soft, vivid
    diffusion = real.copy()
...
# 4️⃣ VAE — slightly stronger blur + mild noise
    # 4️⃣ VAE — smoother, warm tone, slightly blurred but not gray
    vae = real.copy()
    ...
    return sketch_resized, real, pix2pix, diffusion, transformer, vae, f"✅ Generated simulated outputs for `{fname}`!", gr.update(visible=True)
...
with gr.Blocks(css=custom_css, theme=gr.themes.Soft()) as demo:
    ...
    with gr.Column(elem_id="main-container"):
        ...
        with gr.Column(elem_classes="sidebar-wrapper"):
            ...
            <p><b>Pix2Pix</b> – Most realistic output</p>
            <p><b>Diffusion</b> – Artistic and smooth</p>
            <p><b>Transformer</b> – Sharp and stylized</p>
            <p><b>VAE</b> – Noisy and low clarity</p>
            </div>
            """)

        # === Main Content ===
        with gr.Column(elem_classes="main-content"):
            gr.HTML("<h1 style='padding:2rem 0 0 2rem;'>🎨 AI Sketch-to-Image Generator</h1>")
            gr.HTML("<p style='padding-left:2rem;'>Upload your sketch and let AI create photorealistic versions</p>")

            with gr.Column(elem_classes="upload-section"):
                gr.HTML("<h3>📤 Upload Your Sketch</h3>")
                upload_input = gr.File(label="Choose a sketch", file_types=["image"], type="filepath")

            generate_btn = gr.Button("🚀 Generate Images", elem_id="generate-btn")
    ...
    def wrapper(file_obj):
        if file_obj is None:
            return None, None, None, None, None, None, "⚠️ Please upload a file.", gr.update(visible=False), None
...
with gr.Blocks(css=custom_css, theme=gr.themes.Soft()) as demo:
    ...
    with gr.Column(elem_id="main-container"):
        ...
        with gr.Column(elem_classes="sidebar-wrapper"):
            ...
            # Navigation Buttons
            upload_btn = gr.Button("📤 Sketch Upload & Generate", elem_classes="sidebar-item-button active", scale=1)
            comparison_btn = gr.Button("📊 Model Comparison & Analysis", elem_classes="sidebar-item-button", scale=1)
            ...
            <div style='margin-top:3rem;'>
                <div style='padding: 1.5rem; background: linear-gradient(135deg, rgba(96, 165, 250, 0.08), rgba(168, 85, 247, 0.08)); border: 1px solid rgba(96, 165, 250, 0.15); border-radius: 0.875rem;'>
                    <b style='color: #e0f2fe; display: block; margin-bottom: 1rem;'>🎨 Available AI Models</b>
                    <p style='margin: 0.4rem 0; font-size: 0.85rem; color: #cbd5e1;'>🥇 <b>Pix2Pix</b> - Most Realistic</p>
                    <p style='margin: 0.4rem 0; font-size: 0.85rem; color: #cbd5e1;'>🎨 <b>Diffusion</b> - Artistic & Smooth</p>
                    <p style='margin: 0.4rem 0; font-size: 0.85rem; color: #cbd5e1;'>✨ <b>Transformer</b> - Sharp & Stylized</p>
                    <p style='margin: 0.4rem 0; font-size: 0.85rem; color: #cbd5e1;'>🔧 <b>VAE</b> - Experimental</p>
                </div>
            </div>
            """)
        ...
        with gr.Column(elem_classes="main-content"):
            ...
            with gr.Column(visible=True) as upload_page:
                with gr.Column(elem_classes="header-card"):
                    gr.HTML("<h1>🎨 AI Sketch-to-Image Generator</h1>")
                ...
                generate_btn = gr.Button("🚀 Generate Images", elem_id="generate-btn", scale=1)
                ...
                with gr.Column(elem_classes="results-section", visible=False) as results_container:
                    ...
                    model_dropdown = gr.Dropdown(
                        choices=["Pix2Pix (Best)", "Diffusion Model", "Transformer", "VAE"],
                        value="Pix2Pix (Best)",
                        label="🔄 Compare Different Models"
                    )
                    model_output = gr.Image(label="Selected Model Output", height=512)

            # === COMPARISON PAGE ===
            with gr.Column(visible=False, elem_classes="comparison-section") as comparison_page:
                gr.HTML("<h1 class='comparison-title'>📊 Model Comparison & Evaluation</h1>")

                # Top Stats Cards
                gr.HTML("""
                <div class='stat-cards-row'>
                    <div class='stat-card'>
                        <h3>🏆 Best PSNR Score</h3>
                ...
                <table class='eval-table'>
                    ...
                    <tbody>
                        <tr style='background: rgba(16, 185, 129, 0.05);'>
                            <td><span class='rank-badge rank-1'>🥇 Pix2Pix</span></td>
                        ...
                        <tr>
                            <td><span class='rank-badge rank-2'>🥈 VAE</span></td>
                            <td>24.2</td>
                            <td>0.40</td>
                            <td>0.36</td>
                            <td>0.74</td>
                        ...
                        <tr style='background: rgba(96, 165, 250, 0.03);'>
                            <td><span class='rank-badge rank-3'>🥉 Transformer</span></td>
                            <td>21.8</td>
                            <td>0.44</td>
                            <td>0.42</td>
                            <td>0.69</td>
                        ...
                        <tr>
                            <td><span class='rank-badge rank-4'>4️⃣ Diffusion</span></td>
                            <td>19.3</td>
                            <td>0.47</td>
                            <td>0.51</td>
                            <td>0.63</td>
...
def generate_images(sketch, sketch_file):
    ...
    # ============================================================
     ...
     # ================================================
    # 1️⃣ Pix2Pix (GAN-Like High Quality)
    # ================================================
    pix2pix = real.copy()
    ...
    # ================================================
    # 2️⃣ Stable Diffusion (Smooth + Clean + Balanced)
    # ================================================
    diffusion = real.copy()
    ...
    # ==================================================
    # 3️⃣ Vision Transformer  → now uses VAE effect
    ...
    # ==================================================
    # 4️⃣ VAE (Softer Blur + More Clear Face + Washed Out)
    # ==================================================
    vae = real.copy()

    # lighter blur than before (was 9 → now 4)
