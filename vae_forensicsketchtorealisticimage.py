...
class Encoder(nn.Module):
    def __init__(self, in_channels=3, img_size=128, latent_dim=256):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, 64, 4, 2, 1),   # 64 × 64 × 64
            nn.ReLU(True),
...
class Decoder(nn.Module):
    def __init__(self, out_channels=3, img_size=128, latent_dim=256):
        ...
        self.net = nn.Sequential(
            ...
            nn.ConvTranspose2d(64, out_channels, 4, 2, 1),  # 3 × 128 × 128
            nn.Tanh()
        )
...
def find_resume_checkpoint(checkpoint_dir):
    ...
    if os.path.exists(autosave):
        print("✅ Resuming from autosave checkpoint.")
        return autosave

    latest_epoch = find_latest_epoch_ckpt(checkpoint_dir)
    if latest_epoch:
        print("✅ Resuming from:", latest_epoch)
        return latest_epoch
...
def train_vae(model, train_loader, val_loader, epochs, lr, recon_weight, kl_weight,
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for i, (sketches, real, _) in enumerate(pbar):
            ...
            # --------------------------------------
            # ✅ AUTOSAVE CHECKPOINT EVERY 5 MINUTES
            # --------------------------------------
            if time.time() - last_autosave_time > AUTOSAVE_INTERVAL:
                ...
                print(f"\n✅ Autosaved checkpoint at: {autosave_path}")
                last_autosave_time = time.time()
...
def find_resume_checkpoint(save_dir):
    ...
    if os.path.exists(autosave):
        print("✅ Found autosave checkpoint. Will resume training.")
        return autosave
    else:
        print("⚠️ No autosave found. Training will start from epoch 1.")
        return None
...
def train_vae(model, train_loader, val_loader, epochs, lr,
    ...
    if resume_ckpt is not None and os.path.exists(resume_ckpt): # Added existence check
        ...
        try: # Added try-except block for robustness
            ...
            print(f"⚠️ Failed to load checkpoint {resume_ckpt}: {e}")
            print("⚠️ Starting training from epoch 1.")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        # -------------------------------------------------------------
        # ✅ Auto-delete old checkpoints (keep only last 3)
        # -------------------------------------------------------------
        ...
        print(f"✅ Autosaved checkpoint: {autosave_path}")
...
for f in os.listdir(ckpt_dir):
    if f.startswith("vae_epoch_"):
        ...
        if epoch_num < 43:   # keep only last 2–3 checkpoints
            os.remove(os.path.join(ckpt_dir, f))

print("✅ Old checkpoints deleted safely.")
...
def load_checkpoint(fpath, model, optimizer=None):
    ...
    return epoch, ck       # ✅ MUST return TWO values
...
def train_vae(model, train_loader, val_loader, epochs, lr,
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        # -------------------------------------------------------------
        # ✅ Auto-delete older checkpoints (keep only last 3)
        # -------------------------------------------------------------
        ckpts = sorted([f for f in os.listdir(save_dir) if f.startswith("vae_epoch_")])
        if len(ckpts) > 3:
            old = ckpts[:-3]
...
def find_latest_ckpt(folder):
    ...
    if os.path.exists(autosave):
        print("✅ Found autosave checkpoint:", autosave)
        return autosave
    ...
    if epoch_files:
        ...
        if os.path.exists(latest_epoch_ckpt): # Added existence check
            print("✅ Found latest epoch checkpoint:", latest_epoch_ckpt)
            return latest_epoch_ckpt
        else:
            print(f"⚠️ Latest epoch checkpoint found in list but not on disk: {latest_epoch_ckpt}")


    print("⚠️ No valid checkpoint found.") # Added message
    return None
...
plt.imshow(final_grid.permute(1, 2, 0))   # CHW → HWC
plt.axis("off")
...
print("✅ Saved comparison image at:", comp_path)
