...
import torchvision.utils as vutils     # 🔥 required for make_grid
...
# UNet backbone from diffusers — conditional via concatenation
unet = UNet2DModel(
...
with torch.no_grad():
    ...
    for t in reversed(range(noise_scheduler.config.num_train_timesteps)):
        # 🔹 use scalar for scheduler
        t_tensor = torch.tensor(t, device=device, dtype=torch.long)
        # 🔹 batch tensor for model input
    ...
    print("🖼️ Saved visual sample:", save_path)
...
unet.load_state_dict(ckpt["model_state_dict"])
print("✅ Loaded trained UNet from checkpoint")
...
with torch.no_grad():
    ...
    print("🖼️ Saved prediction:", save_path)
...
# ------------------------------------------------
# Helper: convert tensor → image and save
# ------------------------------------------------
...
print("✅ LPIPS model loaded (AlexNet)")
...
with torch.no_grad():
    for i in range(5):
        ...
        save_image((predicted + 1) / 2, save_path)   # convert to 0–1 for saving
        print(f"🖼️ Saved predicted image {i+1} → {save_path}")
...
unet.load_state_dict(ckpt["model_state_dict"])
print("✔ Loaded UNET weights from checkpoint 150")
